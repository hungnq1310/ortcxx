...
BlazeFace::BlazeFace(const std::string& model_path)
    : Pipeline(model_path) {
...
void BlazeFace::initModel(const char* tfliteModel, long modelSize) {
...
std::vector<float> BlazeFace::preprocess(const std::vector<float>& input) {
    // Default implementation: return the input as is
    std::cout << "BlazeFace Preprocessing..." << std::endl;
    return input;
}

std::vector<float> BlazeFace::postprocess(const std::vector<float>& input) {
    // Default implementation: return the input as is
